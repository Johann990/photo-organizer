...
### 3. 事件夾命名一致化 / Consistent event-folder naming (single + multi-day)

統一單日與「單一事件連續多日」的命名為「ISO 日期 + 空格 + 事件」,多日多帶 `(Nd)`:
```
單日 single-day:  舊 {YYYY-MM-DD}_{event}            → 新 {YYYY-MM-DD} {event}        → 2012-09-08 上海
多日 multi-day:   舊 {start}_{span}d_{event}          → 新 {start:%Y-%m-%d}({N}d) {event} → 2005-08-06(9d) 蒙古
```
- 分隔一律用**空格**(`_` → ` `);多日跨度以 `({N}d)` 緊接日期。
- 無 event 名時 → 單日 `{YYYY-MM-DD}`、多日 `{YYYY-MM-DD}({N}d)`(省略事件段)。
- **影響所有單日與連續多日事件**(標記與未標記皆然)。
- **主題 / subject 不適用**:主題事件日期**不連續、跨度大**,維持 `{event}/{YYYY}/`(不帶日期前綴)。
- 既有測試(`test_subject_collection`、`test_folder_overrides_plan` 等)斷言舊格式者需更新為新格式(屬刻意行為變更)。
...
| 情況 | 落點 |
|------|------|
| **多日 + per_day_split=1** | `{base}/{年}/{YYYY-MM-DD}({N}d) {event}/{mmdd}/{原檔名}` |
| 多日(未標記) | `{base}/{年}/{YYYY-MM-DD}({N}d) {event}/{原檔名}`(扁平,新夾名) |
| 單日 | `{base}/{年}/{YYYY-MM-DD} {event}/{原檔名}`(新命名,底線→空格) |
| subject | `{base}/{event}/{年}/{原檔名}`(不變) |
| 無事件 / 無日期 | 不變 |

- `{年}` = 事件起始年;`{YYYY-MM-DD}`/`{N}d`/`{event}` = 事件起始日 / 跨度 / 解析名(可被 `event_name` override)。
